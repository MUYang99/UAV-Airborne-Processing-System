...
private:
	...
	ros::ServiceClient send_data_to_remote_device_service;//透传服务
...
private:
	...
	//start Mission
    CallBackHandler startMission1Callback;//mission1 cmdID = 101
    CallBackHandler startMission2Callback;//mission2 cmdID = 102
    //for test
    CallBackHandler startMission3Callback;//mission3 cmdID = 103
    CallBackHandler startMission4Callback;//mission4 cmdID = 104
    CallBackHandler startMission5Callback;//mission3 cmdID = 105
    CallBackHandler startMission6Callback;//mission4 cmdID = 106
   //shutdown
    CallBackHandler stopMissionCallback;//stopmission cmdID = 107
	...
	void mobile_data_push_info_callback(dji_sdk::TransparentTransmissionData information)
	{
		...
		mobile_new_data = 1;
		cout<<mobile_data.data[0]<<mobile_data.data[1]<<mobile_data.data[2]<<mobile_data.data[3]<<endl;		
		int entry = mobile_data.data[0];
		int cmdID = (int)((char)(mobile_data.data[1])-'0')*100+(int)((char)(mobile_data.data[2])-'0')*10+(int)((char)(mobile_data.data[3])-'0');//uint32_t8
				...
				if (virtualRCTestCallback.callback)
			{
             ...
             case 100:
              if (obtainControlCallback.callback)
				...
				case 101:
				if (startMission1Callback.callback)
			    {
			     startMission1Callback.callback(this);
				}
				break;

            case 102:
                if (startMission2Callback.callback)
			    {
                 startMission2Callback.callback(this);
				}
				break;
            case 103:
                if (startMission3Callback.callback)
			    {
                 startMission3Callback.callback(this);
				}
				break;
            case 104:
                if (startMission4Callback.callback)
			    {
                 startMission4Callback.callback(this);
				}
				break;

            case 105:
                if (startMission5Callback.callback)
			    {
                 startMission5Callback.callback(this);
				}
				break;
            case 106:
                if (startMission6Callback.callback)
			    {
                 startMission6Callback.callback(this);
				}
				break;

			    //stopOpentldCallBack
                case 107:
				if (stopMissionCallback.callback)
			    {
			     stopMissionCallback.callback(this);
				}
				break;
...
public:
    ...
    //set任务回调 in client.cpp
    void setStartMission1Callback(DJIDrone::CallBack userCallback, UserData userData)
	{
        startMission1Callback.callback = userCallback;
        startMission1Callback.userData = userData;
	}//missioin1 cmdID = 101

    void setStartMission2Callback(DJIDrone::CallBack userCallback, UserData userData)
    {
        startMission2Callback.callback = userCallback;
        startMission2Callback.userData = userData;
    }//missioin2 cmdID = 102

    void setStartMission3Callback(DJIDrone::CallBack userCallback, UserData userData)
    {
        startMission3Callback.callback = userCallback;
        startMission3Callback.userData = userData;
    }//missioin3 cmdID = 103

    void setStartMission4Callback(DJIDrone::CallBack userCallback, UserData userData)
    {
        startMission4Callback.callback = userCallback;
        startMission4Callback.userData = userData;
    }//missioin4 cmdID = 104

    void setStartMission5Callback(DJIDrone::CallBack userCallback, UserData userData)
    {
        startMission5Callback.callback = userCallback;
        startMission5Callback.userData = userData;
    }//missioin5 cmdID = 105

    void setStartMission6Callback(DJIDrone::CallBack userCallback, UserData userData)
    {
        startMission6Callback.callback = userCallback;
        startMission6Callback.userData = userData;
    }//missioin6 cmdID = 106
	
	//任务结束回调
	void setStopMissionCallback(DJIDrone::CallBack userCallback, UserData userData)
	{
		stopMissionCallback.callback = userCallback;
		stopMissionCallback.userData = userData;
	}//missioin6 cmdID = 107
	...
	//在client里面调用就行了 
	bool sendDataToMobile(const std::vector<uint8_t> &data_,size_t len)
	{
           dji_sdk::SendDataToRemoteDevice sendData;
           sendData.request.data = data_;
           cout<<"len"<<len<<endl;
	   return  send_data_to_remote_device_service.call(sendData) && sendData.response.result;
	}
